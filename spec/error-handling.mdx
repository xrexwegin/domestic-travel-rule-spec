...
| API | 錯誤回應中的識別碼欄位 |
|------|------------------------|
| `POST /address/verify`（地址所有權查詢） | `request_id` |
| `POST /transfer`（發送 Travel Rule 資料） | `transfer_id` |
| `POST /transfers/{id}/confirm`（確認交易） | `transfer_id` |
| `PATCH /transfers/{id}`（更新交易狀態） | `transfer_id` |
| `GET /transfers/{id}`（查詢交易狀態） | `transfer_id` |

例如 `POST /transfer` 發生錯誤時：
